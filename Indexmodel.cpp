...
int Crawler::crawler_crawl_page(const string& url, string& pageContent)
{
	...
	if(ec)
	{
		//system_error异常类是error_code类的一个适配器，它的构造函数接受error_code类型的参数，可把
		//该类型应用到C++的异常处理机制之中
...
void Crawler::crawler_main_loop(string url,ThreadPool *pool)
{
	...
	//关键词匹配
	if(m_matchWordIndex == true) 
	{
		if(crawler_match_word(url,pageContent) < 0)
		{
	//		cout<<"关键词不匹配"<<endl;
			return;
		}
...
{
	//由于所有的Crawler实例都要共享static变量，因此若有一实例存在则它是唯一的
	if(m_crawlerIsExited == true)
	{
...
/****************************************
函数：crawler_match_word
描述：关键字匹配函数
参数：
1、url：网址
2、pageContent：网页内容
返回值：
1、int：函数调用成功返回0，失败返回-1
****************************************/
int Crawler::crawler_match_word(const string& url,const string& pageContent)
{
	...
	while((posTagAfter = find_html_tag(fileContent,tagAfter,posStart))!=string::npos)
	{
		...
		if((posTagAfter-posTagBefore-tagBefore.size() >= 1) && (tagBefore.find("<script") == string::npos) && (tagBefore.find("<style") == string::npos))
		{
			...
			text.append(textSegment).append(" ");     //每段正文之间以空格分隔
	...
	if(!isPureEnglish)
	{
		...
		if(text.find(Key_Word) != string::npos)
		{
			return 0;
		}
		else
	...
	//利用tokenizer进行纯英文字符匹配
	//将剔除标签后的正文内容进行分词，并形成分词列表，对于相同的词只存储一次
	...
	string keyword_text = Key_Word;
	set<string> KeyWordList; 
	typedef boost::tokenizer< boost::char_separator<char> > keyword_tokenizer;
	boost::char_separator<char> keyword_sep(" ");
	keyword_tokenizer keyword_tokens(keyword_text,keyword_sep);

	//将分词存入KeyWordList列表
	for(keyword_tokenizer::iterator tok_iter=keyword_tokens.begin();tok_iter!=keyword_tokens.end();++tok_iter)
	{
		if(!KeyWordList.count(*tok_iter))
		{
			KeyWordList.insert(*tok_iter);
		}
	}
	...
	{
		set<string>::iterator set_iter = KeyWordList.begin();
		for(int i=0;i<5;i++)
		{
			++ set_iter;
		}

		for(; set_iter!=KeyWordList.end() ; ++ set_iter)
		{
			KeyWordList.erase(*set_iter);
		}
	}

	int count = 0;           //用于记录关键词匹配的个数 

	//判断关键词是否匹配成功
	for(set<string>::const_iterator set_iter = KeyWordList.begin() ; set_iter!=KeyWordList.end(); ++set_iter)
	{
		if(WordList.count(*set_iter))
		{
			++count;
		}
	}
